...
function Header() {
  ...
  return (
    <div className='flex flex-row items-center justify-between w-[96vw] h-[8vh]'>
      ...
      {window_width > 1000 ? <div className='nav_items h9'>
        <Link to="/about">About</Link>
        <Link to="/services">Services</Link>
        <Link to="/blogs">Blogs</Link>
      </div> :
        <div className='flex flex-col justify-start relative'>
          <div className='w-[20px] flex flex-col gap-1 cursor-pointer bg-black' onClick={() => setOpenNavDropdown(!openNavDropdown)}>
